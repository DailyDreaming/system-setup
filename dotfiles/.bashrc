...
export DSS_TEST_ES_PATH=/opt/elasticsearch/bin
...
mkdssb() { git clone https://github.com/HumanCellAtlas/data-store.git && cd data-store && git checkout "$1" && virtualenv -p python3.7 v3nv && . v3nv/bin/activate && pip install -r requirements-dev.txt && source environment; }
...
mkclib() { git clone https://github.com/HumanCellAtlas/dcp-cli.git && cd dcp-cli && git checkout "$1" && virtualenv -p python3.7 v3nv && . v3nv/bin/activate && pip install -r requirements-dev.txt && make install; }
